...
export default function AIQuizGenerator({ user, onAddQuiz }: AIQuizGeneratorProps) {
  ...
  const generateFromText = async () => {
    ...
    if (!includeMultipleChoice && !includeShortAnswer && !includeLongAnswer) {
      toast({
        ...
        description: "Please select at least one question type (Multiple Choice, Short Answer, or Long Answer).",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      console.log("🚀 Generating quiz questions with mixed types and difficulties...")
  ...
  const generateFromImage = async () => {
    ...
    try {
      console.log("🚀 Generating quiz questions from image with mixed types...")
      ...
      if (!data.questions || data.questions.length === 0) {
        throw new Error("No questions were generated from the image. Please try a different image.")
      }
      ...
      <Card className="relative overflow-hidden bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 border-purple-200/50 shadow-2xl">
        ...
        <CardHeader className="relative pb-8">
          ...
          <p className="text-purple-600/80 text-2xl mt-4 font-medium">
            Create comprehensive quizzes with multiple choice, short answer, and long answer questions
          </p>
        </CardHeader>
