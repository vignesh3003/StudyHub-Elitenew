...
import { gamificationService, type UserStats, type Achievement } from "@/lib/gamification-service"
...
export default function UserProfile({ user, studyStats, displayName, onEditProfile }: UserProfileProps) {
  ...
  const refreshUserStats = async () => {
    ...
    try {
      ...
      const freshStats = await gamificationService.getUserStats(user.uid)
      setUserStats(freshStats)
      console.log("✅ User stats refreshed:", freshStats)
    } catch (error) {
      console.error("Error refreshing user stats:", error)
    }
  }

  // Add useEffect to listen for achievement unlocks and refresh stats
  useEffect(() => {
    const unsubscribe = gamificationService.onAchievementUnlocked((achievement) => {
      console.log("🏆 Achievement unlocked, refreshing stats:", achievement.name)
      refreshUserStats()
    })
  ...
  const formatLastActive = (lastActive: any) => {
    if (!lastActive) return "Today"

    try {
      // Check if lastActive has a toDate method (Firestore Timestamp)
      if (lastActive.toDate && typeof lastActive.toDate === "function") {
        return new Date(lastActive.toDate()).toLocaleDateString()
      }
      ...
      console.error("Error formatting lastActive date:", error)
      return "Recently"
    }
  }
    ...
    >
      <CardContent className="p-6">
        <div className="text-center">
          ...
          <div className={`text-6xl mb-4 ${achievement.earned ? "" : "grayscale opacity-50"}`}>{achievement.icon}</div>
  ...
  return (
    <div className="space-y-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
        <TabsList className="grid grid-cols-4 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 shadow-lg rounded-2xl p-1 gap-1">
          <TabsTrigger
            value="profile"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-gray-800">{formatLastActive(userStats?.lastActive)}</span>
                    </div>
                  </div>
                </div>
        ...
        <TabsContent value="achievements" className="space-y-8 animate-in fade-in-50 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {getFilteredAchievements().map((achievement) => renderAchievementCard(achievement))}

            {/* If no achievements match the filter */}
            {getFilteredAchievements().length === 0 && (
              <div className="col-span-3 text-center py-12">
                ...
                <p className="text-gray-500">
                  {rarityFilter
                    ? `No ${rarityFilter} achievements match your filter. Try selecting a different category.`
                    : "Start using the platform to unlock achievements!"}
                </p>
              </div>
            )}
          </div>
