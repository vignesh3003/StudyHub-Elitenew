...
export interface Task {
  ...
  userId: string
  tags?: string[]
  studyPlanId?: string // To link tasks created from study plans
}
...
class TaskService {
  ...
  async addTask(userId: string, taskData: Omit<Task, "id" | "createdAt" | "updatedAt" | "userId">): Promise<string> {
    try {
      ...
      if (taskData.studyPlanId) {
        cleanTaskData.studyPlanId = taskData.studyPlanId
      }
  ...
  // Add multiple tasks from study plan
  async addTasksFromStudyPlan(
    userId: string,
    tasks: Omit<Task, "id" | "createdAt" | "updatedAt" | "userId">[],
    studyPlanId: string,
  ): Promise<string[]> {
    try {
      ...
      for (const taskData of tasks) {
        const taskId = await this.addTask(userId, {
          ...taskData,
          studyPlanId,
        })
        taskIds.push(taskId)
      ...
      console.error("Error adding tasks from study plan:", error)
      throw error
    }
  }
