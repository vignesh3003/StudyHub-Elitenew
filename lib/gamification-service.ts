...
class GamificationService {
  ...
  async getUserStats(userId: string): Promise<UserStats> {
    try {
      ...
      // MIGRATION: Update existing users with new achievement structure
      const needsMigration =
        !data.achievements || data.achievements.length < 25 || !data.achievements.find((a) => a.id === "first-steps")

      if (needsMigration) {
        console.log("Migrating user achievements to new structure...")

        // Get the fresh achievement structure
        const freshStats = await this.initUserStats(userId)

        // Preserve existing progress but update structure
        const migratedAchievements = freshStats.achievements.map((newAchievement) => {
          // Check if user had this achievement before
          const existingAchievement = data.achievements?.find((a) => a.id === newAchievement.id)

          if (existingAchievement) {
            return {
              ...newAchievement,
              earned: existingAchievement.earned,
              progress: existingAchievement.progress,
              date: existingAchievement.date,
            }
          }

          // For new achievements, check if they should be auto-earned
          if (newAchievement.id === "first-steps") {
            return {
              ...newAchievement,
              earned: true,
              date: new Date().toLocaleDateString(),
          ...
          // Check if user qualifies for other achievements based on current stats
          if (newAchievement.id === "first-task" && (data.tasksCompleted || 0) >= 1) {
            return {
              ...newAchievement,
              earned: true,
              date: new Date().toLocaleDateString(),
              progress: newAchievement.maxProgress,
            }
          }

          if (newAchievement.id === "first-study" && (data.studyHours || 0) >= 1) {
            return {
              ...newAchievement,
              earned: true,
              date: new Date().toLocaleDateString(),
          ...
          if (newAchievement.category === "tasks" && newAchievement.maxProgress) {
            return {
              ...newAchievement,
              progress: Math.min(data.tasksCompleted || 0, newAchievement.maxProgress),
            }
          }

          if (newAchievement.category === "study" && newAchievement.maxProgress) {
            const studySessions = Math.floor((data.studyHours || 0) * 2) // Approximate sessions
            return {
              ...newAchievement,
              progress: Math.min(studySessions, newAchievement.maxProgress),
            }
          }
        ...
        // Update the user's data with migrated achievements
        const updatedData = {
          ...data,
      ...
      // Ensure the "First Steps" achievement is always earned for existing users
      if (data.achievements) {
        const firstStepsIndex = data.achievements.findIndex((a) => a.id === "first-steps")
        if (firstStepsIndex !== -1 && !data.achievements[firstStepsIndex].earned) {
          data.achievements[firstStepsIndex] = {
            ...data.achievements[firstStepsIndex],
            earned: true,
            date: new Date().toLocaleDateString(),
          }
  ...
  async recordFlashcardCreated(userId: string, count = 1): Promise<void> {
    try {
      ...
      console.warn("Error recording flashcard creation:", error.message)
